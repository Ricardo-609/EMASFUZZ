...
#define N_FUZZ_SIZE (1 << 21)
  ...
  u32 tainted_len, /*       taint inference info       */
      cur_tainted_len, tainted_seed[TAINT_MODE];

  u8 taint_mode, taint_alone_mode;
...
/* Taint Inference */
u8 taint_inference_stage(afl_state_t *afl, u8 *buf, u8 *orig_buf, u32 len,
                         u8 mode);

u8 taint_fuzz(afl_state_t *afl, u8 *buf, u8 *orig_buf, u32 len, u8 mode);

struct tainted *get_constraint(struct tainted *taint, u8 *buf, u8 *orig_buf,
                               u32 len);

void set_constraint(struct tainted *taint, u8 *buf, u8 *orig_buf, u32 len);
...
u8 cmp_extend_encodingN(afl_state_t *afl, struct cmp_header *h, u128 pattern,
                        u128 repl, u128 o_pattern, u128 changed_val, u8 attr,
                        u32 idx, u32 taint_len, u8 *orig_buf, u8 *buf, u8 *cbuf,
                        u32 len, u8 do_reverse, u8 lvl, u8 *status);

u8 cmp_extend_encoding(afl_state_t *afl, struct cmp_header *h, u64 pattern,
                       u64 repl, u64 o_pattern, u64 changed_val, u8 attr,
                       u32 idx, u32 taint_len, u8 *orig_buf, u8 *buf, u8 *cbuf,
                       u32 len, u8 do_reverse, u8 lvl, u8 *status);

u8 rtn_extend_encoding(afl_state_t *afl, u8 entry, struct cmpfn_operands *o,
                       struct cmpfn_operands *orig_o, u32 idx, u32 taint_len,
                       u8 *orig_buf, u8 *buf, u8 *cbuf, u32 len, u8 lvl,
                       u8 *status);
