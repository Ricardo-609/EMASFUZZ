...
static u32 select_best_child(afl_state_t * afl)
{
  ...
  while(child)
  {
    child->UCT = (child->q - Q_min)/(Q_max - Q_min) + 1.414* sqrt(log((double)(afl->tree_tmp->N)) / ((double)(afl->child_cur->N)));
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  if (afl->symbolic_mode) {
    // check cmplog mode
    if (!afl->shm.cmplog_mode) { FATAL("To support symbolic we need cmplog"); }
    ...
    if (!d && errno == ENOENT) { FATAL("Symbolic path does not exist"); }

    closedir(d);
    // check s2e script
    u8 *fn = alloc_printf("%s/launch-s2e.sh", afl->symbolic_path);
    if (access(fn, F_OK) < 0) { FATAL("Seems that s2e script does not exist"); }
    ck_free(fn);
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
      ...
      // if symbolic is terminated and is ready for synchronizing, we
      // synchronize testcases generated by symbolic engine.
      if (afl->symbolic_mode && afl->ready_for_sync && !afl->stop_soon) {
        ...
        if (d) {
          if (rename(fn, new_fn) < 0) { PFATAL("Rename testcase- failed"); }
...
stop_fuzzing:
  ...
  if (afl->symbolic_mode) fclose(afl->s2e_solving_stat);
