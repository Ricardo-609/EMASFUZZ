...
void maybe_update_plot_file(afl_state_t *afl, u32 t_bytes, double bitmap_cvg,
  ...
  fprintf(afl->fsrv.plot_file,
          ...
          afl->fsrv.total_havocs); /* ignore errors */
...
#define SP20 SP10 SP10
  ...
  if (afl->shm.memlog_mode || afl->shm.cmplog_mode) {
    sprintf(tmp, "%s/%s, %s/%s, %s/%s",
            u_stringify_int(IB(0), afl->stage_finds[STAGE_TAINT_HAVOC]),
    ...
    if (afl->symbolic_mode) {
      sprintf(tmp, "%s/%s/%s",
              u_stringify_int(IB(0), afl->stage_finds[STAGE_SYMBOLIC]),
    ...
    SAYF(SET_G1 bSTG bVR bH cCYA                                   bSTOP
         " taint inference " bSTG bH2 bH bH5 bH10 bH2 bH10 bH2 bH2 bVL "\n");
    ...
    if (afl->taint_alone_mode) {
      sprintf(tmp, "%d/%d/%d", afl->tainted_seed[TAINT_CMP], afl->s2e_hang,
              afl->queued_items);
