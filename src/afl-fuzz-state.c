...
void afl_states_stop(void) {
  ...
  LIST_FOREACH(&afl_states, afl_state_t, {
    ...
    if (el->shm.cmplog_mode) {
      if (el->cmplog_fsrv.child_pid > 0)
        kill(el->cmplog_fsrv.child_pid, el->cmplog_fsrv.kill_signal);
      if (el->cmplog_fsrv.fsrv_pid > 0)
        kill(el->cmplog_fsrv.fsrv_pid, el->cmplog_fsrv.kill_signal);
    }

    if (el->shm.memlog_mode) {
      if (el->memlog_fsrv.child_pid > 0)
        kill(el->memlog_fsrv.child_pid, el->memlog_fsrv.kill_signal);
      if (el->memlog_fsrv.fsrv_pid > 0)
        kill(el->memlog_fsrv.fsrv_pid, el->memlog_fsrv.kill_signal);
    }
  });
}
